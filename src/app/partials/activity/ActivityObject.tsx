                    ...
                    <div className="card-title m-0">
                        <div className="bg-light w-100px">
                            {activity.game?.image ? (
                                ...
                                <img src={toAbsoluteUrl("assets/media/svg/brand-logos/plurk.svg")} alt={'Activity Blank'}/>
                            )}
                        </div>
                    </div>
                    <div className="card-toolbar">
                        {activity.status === 1 &&
                            <span className="badge badge-light-primary fw-bolder me-auto px-4 py-3">Registration</span>
                        }
                        {activity.status === 2 &&
                            <span className="badge badge-light-success fw-bolder me-auto px-4 py-3">Active</span>
                        }
                        {activity.status === 3 &&
                            <span className="badge badge-light-secondary fw-bolder me-auto px-4 py-3">Pending</span>
                        }
                        {activity.status === 4 &&
                            <span className="badge badge-light-danger fw-bolder me-auto px-4 py-3">Closed</span>
                        }
                        {activity.status === 5 &&
                            <span className="badge badge-light-warning fw-bolder me-auto px-4 py-3">Generating Matches</span>
                        }
                    </div>
                </div>
                <KTCardBody className="p-9">
                    ...
                    <div className="d-flex flex-wrap mb-5">
                        <div className="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-7 mb-3">
                            <div className="fs-6 text-gray-800 fw-bolder">{Moment(activity?.registration_dates?.end_date).format("ddd, MMM D YYYY")}</div>
                    ...
                    <div className="h-4px w-100 bg-light mb-5" data-bs-toggle="tooltip" title="This project 50% completed">
                        <ProgressBar className="rounded h-4px" now={60}/>
                    </div>
