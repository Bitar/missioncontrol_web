...
export const PlatformDetail = () => {
  ...
  useEffect(() => {
    getPlatforms().then((response) => {
      ...
      responsePlatforms = responsePlatforms?.filter((platform) =>
        !game?.platforms.some(elem => elem.id === platform.id)
      );
  ...
  const handleSubmit = async () => {
    ...
    await addGamePlatform(game?.id, data).then(() => {
      updateGame();
      let updatePlatforms = platforms?.filter((platform) => platformForm?.platform_id.toString() !== (platform.id?.toString()));
                  ...
                  <div className="row mb-6">
                    ...
                    <div className="col-lg-8 fv-row">
                      <Select
                        ...
                        menuPlacement={"top"}
                        options={platforms?.filter(platform => !game?.platforms.includes(platform))}
                        getOptionLabel={(platform) => platform?.name}
                        getOptionValue={(platform) => platform?.id?.toString() || ""}
                        onChange={(e) => {
                          updateData({
                            platform_id: e?.id
                          }, setPlatformForm, platformForm);
                        }}
                      />
