...
import { isCommunityAdmin, isSuperAdmin } from "../../identity/user/models/User";
...
const GameModes: FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      if (isSuperAdmin(currentUser) || (isCommunityAdmin(currentUser) && isAllowedTo("manage-games"))) {
        setShowManage(true);
      }
    }
  }, [currentUser]);
              ...
              >
                ...
                {gameMode?.description && (
                  <div className="fs-7 fw-bold text-gray-400 mt-auto mb-2">
                    {gameMode?.description.length > 150
                      ? gameMode?.description.substring(0, 150) + "..."
                      : gameMode?.description}
                  </div>
                )}
